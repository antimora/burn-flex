...
pub fn gather<E: Element + Pod + Default + Copy + Send + Sync>(
    ...
    if ndims == 2 {
        ...
        return EmberTensor::new(bytes, Layout::contiguous(indices_shape.clone()), E::dtype());
    }
    ...
    let bytes = Bytes::from_elems(result);
    EmberTensor::new(bytes, Layout::contiguous(indices_shape.clone()), E::dtype())
}
...
pub fn scatter_add<E: Element + Pod + Default + Copy + core::ops::AddAssign + Send + Sync>(
    ...
    let bytes = Bytes::from_elems(result);
    EmberTensor::new(bytes, Layout::contiguous(tensor_shape), E::dtype())
}
...
pub fn select<E: Element + Pod + Default + Copy + Send + Sync>(
    ...
    if ndims == 2 {
        ...
        let bytes = Bytes::from_elems(result);
        return EmberTensor::new(bytes, Layout::contiguous(output_shape), E::dtype());
    }
...
pub fn select_add<E: Element + Pod + Default + Copy + core::ops::AddAssign + Send + Sync>(
    ...
    if ndims == 2 {
        ...
        let bytes = Bytes::from_elems(result);
        return EmberTensor::new(bytes, Layout::contiguous(tensor_shape), E::dtype());
    }
...
pub fn gather_f32(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
    gather::<f32>(tensor, dim, indices)
}

pub fn gather_f64(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
    gather::<f64>(tensor, dim, indices)
}

pub fn gather_i64(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
...
pub fn select_f32(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
    select::<f32>(tensor, dim, indices)
}

pub fn select_f64(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
    select::<f64>(tensor, dim, indices)
}

pub fn select_i64(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
...
pub fn gather_bool(tensor: EmberTensor, dim: usize, indices: EmberTensor) -> EmberTensor {
...
pub fn scatter_or(
    ...
    let bytes = Bytes::from_elems(result);
    EmberTensor::new(bytes, Layout::contiguous(tensor_shape), DType::Bool)
}
...
mod tests {
    ...
    #[test]
    fn test_gather_1d() {
        let tensor = EmberTensor::from_data(TensorData::new(vec![10.0f32, 20.0, 30.0, 40.0], [4]));
        let indices = EmberTensor::from_data(TensorData::new(vec![3i64, 0, 2], [3]));
    ...
    fn test_gather_2d_dim1() {
        ...
        let indices = EmberTensor::from_data(TensorData::new(vec![0i64, 2, 1, 0], [2, 2]));
    ...
    fn test_gather_2d_dim0() {
        ...
        // row 0: [1.0, 2.0, 3.0]
        // row 1: [4.0, 5.0, 6.0]
        let indices = EmberTensor::from_data(TensorData::new(vec![1i64, 0, 1, 0, 1, 0], [2, 3]));
    ...
    fn test_scatter_add_1d() {
        let tensor = EmberTensor::from_data(TensorData::new(vec![0.0f32, 0.0, 0.0, 0.0], [4]));
        let indices = EmberTensor::from_data(TensorData::new(vec![1i64, 2, 1], [3]));
        let value = EmberTensor::from_data(TensorData::new(vec![1.0f32, 2.0, 3.0], [3]));
    ...
    fn test_scatter_add_2d_dim1() {
        ...
        let indices = EmberTensor::from_data(TensorData::new(vec![0i64, 2, 1, 0], [2, 2]));
        let value = EmberTensor::from_data(TensorData::new(vec![1.0f32, 2.0, 3.0, 4.0], [2, 2]));
    ...
    fn test_select_2d_dim0() {
        ...
        let indices = EmberTensor::from_data(TensorData::new(vec![2i64, 0], [2]));
