//! Diagnostic test: compare Ember vs NdArray on permuted (non-contiguous) tensors.
//!
...
type E = burn_ember::Ember;
type N = burn_ndarray::NdArray;

const TOL: f32 = 1e-5;

fn compare(name: &str, ember_data: &TensorData, ndarray_data: &TensorData) -> (f32, f32) {
    let e_vals: Vec<f32> = ember_data.to_vec().unwrap();
    let n_vals: Vec<f32> = ndarray_data.to_vec().unwrap();

    assert_eq!(
        e_vals.len(),
        n_vals.len(),
        "{}: length mismatch (ember={}, ndarray={})",
        name,
        e_vals.len(),
        n_vals.len()
    );
    ...
    for (i, (&e, &n)) in e_vals.iter().zip(n_vals.iter()).enumerate() {
        ...
        if diff > TOL * 10.0 {
            ...
            if sum_diff < 100.0 * TOL {
                eprintln!("  [{name}] idx={i}: ember={e:.8}, ndarray={n:.8}, diff={diff:.8}");
            }
        }
    }
