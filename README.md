...
- **FFT**: Real FFT (rfft) via Cooley-Tukey with complex packing, mixed radix-4/radix-2, compile-time
  twiddle tables, and SIMD butterflies. Within ~2.5x of rustfft while maintaining `no_std` support.
...
| FFT (rfft)        | **within ~2.5x of rustfft** | no_std; rustfft requires std  |
